...
transformed parameters {
  ...
  for (j in 1:J) {
    ...
    beta_j[3, j] = mu_beta2 + sigma[3] * z[3, j]; // Pendiente tiempo
...
model {
  ...
  for (i in 1:N) {
    ...
    if (valid[i] == 1) {
      Omission[counter] ~ bernoulli_logit(beta_j[1, subject] + beta_j[2, subject] * ((alpha[subject, s] - 0.02)/0.98) + beta_j[3, subject] * trial[i]);
      counter += 1;
    }
    ...
    alpha[subject, s] = fmax(0.02, abs(v[subject, s])); 
...
generated quantities {
  ...
  { // local block
    ...
    for (i in 1:N) {
      ...
      if (valid[i] == 1) {
        log_lik[counter] = bernoulli_logit_lpmf(Omission[counter] | beta_j[1, subject] + beta_j[2, subject] * ((alpha[subject, s] - 0.02)/0.98) + beta_j[3, subject] * trial[i]);
        counter += 1;
      }
      
      y_rep[i, 1] = bernoulli_logit_rng(beta_j[1, subject] + beta_j[2, subject] * ((alpha[subject, 1] - 0.02)/0.98) + beta_j[3, subject] * trial[i]);
      y_rep[i, 2] = bernoulli_logit_rng(beta_j[1, subject] + beta_j[2, subject] * ((alpha[subject, 2] - 0.02)/0.98) + beta_j[3, subject] * trial[i]);
